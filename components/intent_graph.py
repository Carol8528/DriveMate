...
SCENARIO_LABELS = {
    ...
    "human_support": "人工客服",
    "trip_status": "行程状态",
    "family_long_trip": "家庭长途出行",
    "airport_transfer": "赶飞机行程",
    "robotaxi_dropoff": "临时安全下车",
}
...
DEFINITIONS: Dict[str, Dict[str, Any]] = {
    ...
    "fatigue": {
        ...
        "signals": [
            ...
            ("一般疲劳表达", r"疲劳|困倦|清醒(一下)?|有点困|太困|困了|很困|好困|太累|休息区|眯一会", 4.0),
            ("疲劳下坚持驾驶", r"还能坚持|坚持.*(继续|开)|继续开|撑(一|下)|赶路|硬撑", 3.0),
        ],
    ...
    "family_long_trip": {
        "mode": "driver",
        "signals": [
            ("儿童同行", r"带孩子(去|到|出远门|旅游)|带娃(去|到)|孩子.*一起(去|到)", 5.0),
            ("到达时间约束", r"[一二两三四五六七八九十\d]{1,2}\s*点(半|[一二三四五六七八九十\d]{1,2}\s*分?)?(前|之前)?(到|前到|到达)|别.*迟到", 4.0),
            ("防晕车需求", r"晕车", 4.0),
            ("途中用餐", r"吃个饭|吃饭|用餐|吃点东西", 3.5),
            ("长途出行", r"长途|出去玩|旅游|自驾游", 3.0),
        ],
        "anti": [("明确否定长途出行", r"不出远门|不去旅游", -8.0)],
    },
    "airport_transfer": {
        "mode": "driver",
        "signals": [
            ("航班约束", r"航班|赶飞机|飞机", 7.0),
            ("机场目的地", r"机场", 5.0),
            ("车内会议", r"开个会|开会|会议|电话会", 4.0),
            ("平稳路线", r"平稳|稳一点|稳一些|别.*颠", 3.5),
        ],
        "anti": [("明确否定赶飞机", r"不赶飞机|不用去机场", -8.0)],
    },
    "robotaxi_dropoff": {
        "mode": "robotaxi",
        "signals": [
            ("提前下车请求", r"我要下车|现在下车|提前下车|就在这(里|儿)下|放我下来", 8.0),
            ("终止行程", r"不用到目的地|不到目的地|不去了", 6.0),
            ("就地停车请求", r"就(在)?这(里|儿)停|靠边停|停车我要下", 5.0),
        ],
        "anti": [("明确否定下车", r"不下车|别停车|不用停", -9.0)],
    },
}
