...
> 当前仓库是演示版本。车辆状态、DMS、语音及环境数据均为可复现的本地模拟数据，不代表已完成量产车、真实订单平台或真实传感器接入。

## 核心能力

- 双场景交互：支持“车主自驾”和“Robotaxi 乘客”两种身份与权限边界。
- 复杂场景拆解：一句话多目标需求（家庭长途出行、赶飞机行程、Robotaxi 临时下车）由 ComplexScenarioDecomposer 拆成带依赖关系的子目标 DAG，再交给确定性链统一编排；理解需求不等于获得执行权限。
- 多模态融合：联合车辆、驾驶员、语音和环境快照生成可解释判断。
- 跨域任务编排：覆盖座舱舒适、导航充电、安全监测、Robotaxi 行程和人工应急等工具域。
- 风险确认：高风险写操作进入待确认状态；确认前重新校验关键状态，状态发生实质变化时拒绝沿用旧授权。多阶段授权链（如先靠边停车、再解锁车门）按依赖顺序逐段确认。
...
业务编排服务（backend_service.py）
        ...
        ├─ ComplexScenarioDecomposer（多目标复杂场景 → 子目标 DAG）
...
推荐演示场景包括四个复杂场景（带孩子长途出行的多目标拆解、赶飞机行程的并存约束、疲劳但要求继续开时的理解权/执行权分离、Robotaxi 临时下车的分步授权与开门前状态重检），以及疲劳驾驶安全处置、带孩子出行的座舱调整、路线导航，以及 Robotaxi 行程状态查询和安全上下车点处理。四个复杂场景在前端快捷场景中各有一个按钮，可直接复现。
